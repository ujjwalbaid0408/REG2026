...
\node[io] (wsi) {WSI\\\scriptsize tiled / striped TIFF};
\node[box,right=of wsi] (tile) {Bounded-budget\\tiler};
\node[enc,right=of tile] (conch) {Encoder(s)\\\scriptsize CONCH 512-d\\\scriptsize ($+$UNI2-h 1536-d)};
...
\begin{tikzpicture}[
  ...
  t/.style={draw,rounded corners,minimum width=12mm,minimum height=6mm,font=\scriptsize,align=center},
  cc/.style={t,fill=green!8}, uu/.style={t,fill=cyan!10},
  op/.style={draw,circle,inner sep=1pt,font=\scriptsize,fill=blue!6},
  b/.style={draw,rounded corners,minimum width=16mm,minimum height=7mm,font=\scriptsize,align=center,fill=blue!5},
  h/.style={draw,rounded corners,minimum width=15mm,minimum height=7mm,font=\scriptsize,align=center,fill=orange!10},
  arr/.style={-{Latex[length=1.8mm]}}
]
\node[t,fill=black!4] (tile) {tile $t_i$\\\scriptsize $224^2$};
\node[cc,above right=4mm and 10mm of tile] (conch) {CONCH\\\scriptsize 512-d};
\node[uu,below right=4mm and 10mm of tile] (uni) {UNI2-h\\\scriptsize 1536-d};
\node[op,right=18mm of tile] (cat) {$\|$};
\node[b,right=of cat] (fuse) {fused $e_i$\\\scriptsize 2048-d};
\node[b,right=of fuse] (att) {Gated-attn\\MIL};
\node[h,above right=3mm and 9mm of att] (ho) {organ (10)};
\node[h,below right=3mm and 9mm of att] (hd) {dx (77)};
\draw[arr] (tile)--(conch); \draw[arr] (tile)--(uni);
\draw[arr] (conch)--(cat); \draw[arr] (uni)--(cat);
\draw[arr] (cat)--(fuse); \draw[arr] (fuse)--(att);
\draw[arr] (att)--(ho); \draw[arr] (att)--(hd);
\node[font=\scriptsize\itshape,above=0.5mm of conch] {frozen};
\node[font=\scriptsize\itshape,below=0.5mm of uni] {frozen};
\end{tikzpicture}
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fusion_curves.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.92\linewidth]{figures/test_components.pdf}
